...
int main(int argc, char *argv[]){
    ...
    std::string kDir = "/home/jqzlca/workspace/3D-Eye-Tracker-master/3D-Eye-Tracker-master/";
    ...
    try{
        ...
        case InputMode::CAMERA:
            ...
            eyecams[0] = std::make_unique<eye_tracker::EyeCamera>("Pupil Cam1 ID0");
            eyecams[1] = std::make_unique<eye_tracker::EyeCamera>("Pupil Cam1 ID2");
#endif
            eye_model_updaters[0] = std::make_unique<eye_tracker::EyeModelUpdater>(focal_length, 5, 0.5);
    ...
    while (is_run) {
        ...
        for (size_t cam = 0; cam < kCameraNums; cam++) {
            ...
            rr_pf=pupilDetector.run(img_grey);
            for(auto it:pupilDetector.out23DEyeTracker){
                inlier_pts.emplace_back(it);
            }
            bool is_pupil_found=true;
